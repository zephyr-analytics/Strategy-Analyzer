...
class MonteCarloApp(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        ctk.CTkEntry(sidebar, textvariable=ctk.StringVar(value=self.data_models.start_date)).pack(pady=(0, 5))

        ctk.CTkLabel(sidebar, text="End Date:", font=bold_font).pack(pady=(0, 0))
        ctk.CTkEntry(sidebar, textvariable=ctk.StringVar(value=self.data_models.end_date)).pack(pady=(0, 10))

        ctk.CTkLabel(sidebar, text="Cash Ticker:", font=bold_font).pack(pady=(0, 0))
        ctk.CTkEntry(sidebar, textvariable=ctk.StringVar(value=self.data_models.cash_ticker)).pack(pady=(0, 5))

        ctk.CTkLabel(sidebar, text="Bond Ticker:", font=bold_font).pack(pady=(0, 0))
        ctk.CTkEntry(sidebar, textvariable=ctk.StringVar(value=self.data_models.bond_ticker)).pack(pady=(0, 10))

        ctk.CTkLabel(sidebar, text="Trading Frequency:", font=bold_font).pack(pady=(0, 0))
        trading_options = ["Monthly", "Bi-Monthly"]
        ctk.CTkOptionMenu(sidebar, values=trading_options, variable=ctk.StringVar(value=self.data_models.trading_frequency)).pack(pady=(0, 10))
        ...
        ctk.CTkOptionMenu(sidebar, values=weighting_options, variable=ctk.StringVar(value=self.data_models.weighting_strategy)).pack(pady=(0, 10))

        ctk.CTkLabel(sidebar, text="SMA Window (days):", font=bold_font).pack(pady=(0, 0))
        sma_windows = ["21", "42", "63", "84", "105", "126", "147", "168", "210"]
        ctk.CTkOptionMenu(sidebar, values=sma_windows, variable=ctk.StringVar(value=self.data_models.sma_window)).pack(pady=(0, 10))

        ctk.CTkButton(sidebar, text="Select Asset Weights File",
        ...
        ctk.CTkOptionMenu(right_sidebar, values=theme_options,
                           variable=ctk.StringVar(value=self.data_models.theme_mode), command=self.change_theme).pack(pady=(0, 20), padx=(10, 10))
    ...
    def create_backtesting_tab(self, tab_control):
        ...
        ctk.CTkLabel(backtesting_tab, text="Portfolio Backtesting",
    ...
    def create_monte_carlo_tab(self, tab_control, bold_font):
        ...
        ctk.CTkLabel(monte_carlo_tab, text="Monte Carlo Simulation",
                     font=ctk.CTkFont(size=20, weight="bold")).pack(pady=10)
        ctk.CTkLabel(monte_carlo_tab, text="Number of Simulations:",
                     font=bold_font).pack(pady=0)
        ctk.CTkEntry(monte_carlo_tab, textvariable=ctk.StringVar(value=self.data_models.num_simulations)).pack(pady=(0, 10))
        ctk.CTkLabel(monte_carlo_tab, text="Simulation Horizon (years):",
                     font=bold_font).pack(pady=0)
        ctk.CTkEntry(monte_carlo_tab, textvariable=ctk.StringVar(value=self.data_models.simulation_horizon)).pack(pady=(0, 10))
        ctk.CTkButton(monte_carlo_tab, text="Run Simulation",
    ...
    def create_signals_tab(self, tab_control, bold_font):
        ...
        ctk.CTkLabel(signals_tab, text="Generate Portfolio Signals",
                     font=ctk.CTkFont(size=20, weight="bold")).pack(pady=10)
        ...
        ctk.CTkButton(signals_tab, text="Generate Signals",
                      command=lambda: self.run_signals_and_display(signal_date.get())).pack(pady=10)
    ...
    def display_asset_weights(self):
        ...
        self.bottom_text = ctk.CTkLabel(self.bottom_text_frame,
